library(Seurat)
library(ggplot2)
ad <- readRDS("./snc_figout.Rdata")
Idents(ad) <- "label_5"
re_levels <- c('Astrocytes','Excitatory','Inhibitory','GABA', # nolint
                'HdNs','DaNs','OPC','MOL','Microglia','Immune','Endothelial','Vasc','Ependymal') # nolint
ad$label_5 <- factor(ad$label_5,levels = re_levels)
Idents(ad) <- 'label_5' # nolint
...
ftr_Excitatory <- c('SLC17A6','FAM19A1')
...
ftr_MOL <- c('MOBP','MOG')
violin_features <- c(ftr_Astrocytes,ftr_Excitatory,ftr_Inhibitory,ftr_GABA,ftr_HdNs,ftr_DaNs,ftr_OPC,ftr_MOL,ftr_Microglia,ftr_Immune,ftr_Endothelial,ftr_Vasc,ftr_Ependymal)# nolint
...
Idents(ad) <- "label_5" 
re_levels <- c('Astrocytes','Excitatory','eSPN','iSPN','dSPN','PVALB/TH IN','SST/NPY IN','CHAT IN','OPC','NFOL','MOL','Microglia','Immune','Endothelial','Vasc') # nolint
ad$label_5 <- factor(ad$label_5,levels = re_levels)
Idents(ad) <- 'label_5' # nolint
...
ftr_Excitatory <- c('SLC17A7', 'SATB2')  # nolint
ftr_Endothelial <- c('FLT1')  # nolint
ftr_Vasc <- c('DCN')  # nolint
ftr_MOL <- c('MOBP','MOG') # nolint
ftr_CHAT_IN <- c('CHAT','NEB')  # nolint
violin_features <- c(ftr_Astrocytes,ftr_Excitatory,ftr_eSPN,ftr_iSPN,ftr_dSPN,ftr_PVALB_TH_IN,ftr_SST_NPY_IN,ftr_CHAT_IN,ftr_OPC,ftr_MOL,ftr_Microglia,ftr_Immune,ftr_Endothelial,ftr_Vasc) # nolint
